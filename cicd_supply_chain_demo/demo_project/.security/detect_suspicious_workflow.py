"""
[탐지 스크립트] CI/CD 워크플로우 YAML에서 "시크릿을 외부로 유출하는 스텝"을
정규식/구조 기반 휴리스틱으로 탐지합니다.

판단 기준:
  같은 스텝(run 블록) 안에
    (a) 네트워크로 나가는 명령(curl/wget/Invoke-WebRequest/Invoke-RestMethod/nc/
        python urllib.request 등)이 있고
    (b) `${{ secrets.XXX }}` 형태로 시크릿을 참조하는 부분이 같이 있으면
  -> "시크릿 유출 의심" 으로 HIGH 등급 플래그.

  네트워크 명령만 있고 시크릿 참조가 없으면(예: 공개 API 조회) 정상으로 간주해서
  플래그하지 않습니다 - 이게 오탐률을 낮추는 핵심 포인트입니다.
